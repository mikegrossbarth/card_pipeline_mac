...
class AppSharedWorkflowLogicTests(unittest.TestCase):
    ...
    def test_edit_inventory_row_updates_visible_fields_and_rebuilds_key(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            record = dummy._normalize_inventory_record({"assigned_person": "Lucas", "item_type": "Raw", "item_id": "RAW-TEAM-20260620-0001", "source_sheet": "Old.xlsx", "status": "Active"})
            ...
            try:
                ...
                self.assertEqual(len(rows), 1)
                edited = rows[0]
                self.assertEqual(edited["item_type"], "Graded")
                self.assertEqual(edited["item_id"], "")
